...
The following rules apply to all analysis, coding, refactoring, maintenance, optimization, bug fixing, and new feature development in this repository.
...
- Adhere closer to SOLID principles
- Implement changes in a controlled, clear, and testable manner

These principles are concrete, project-specific applications of clean-code standards.
...
### Project-Specific Constant & Naming Rules

#### Magic Numbers & Magic Strings
The use of raw literals in code should be minimized.

Values of the following types must be extracted into centralized constants/configs:
- Thresholds (e.g., confidence, matching, or similarity scores like `0.75`, `0.80`)
- Field names (e.g., `"pdf_text"`, `"full_name"`)
- Output/result keys and keys in dictionaries/JSON
- Repeated string literals
- Regex patterns or pattern parts
- File, folder, and directory paths
- Decision/logic parameters
- Status names and constant messages

##### Centralization Targets
Depending on context, use appropriate modules for constants:
- `src/helpers/constants.py` (for general pipeline and business constants)
- Dedicated definitions/configurations (e.g., `definitions.py`, `field_names.py`, `thresholds.py`, `config.py`, or `schema.py`)

##### Constant Naming
Constant names must be descriptive and meaningful.
...
Preferred:
- `NAME_MATCH_CONFIDENCE_THRESHOLD`
- `PDF_TEXT_FIELD`
- `DEFAULT_OUTPUT_DIRECTORY`

##### Category Separation
Do not place all constants in a single flat file without organization. Maintain clear categories:
- **Domain Constants:** Values that define business logic.
- **Field & Schema Constants:** Column headers, JSON fields, database keys.
- **Decision Thresholds:** Similarity/matching scores and filtering criteria.
- **Technical Config:** Runtime or environment settings.

Do not force environment variables for values that are not configuration. The goal is meaningful centralization, not unnecessary abstraction.

#### Naming Conventions (Variables & Functions)
Names must be clear, intention-revealing, and domain-centric:
- Avoid meaningless short abbreviations.
- Minimize generic names like `data`, `temp`, `misc`, `value`, `obj`.
- Variable names should describe what the variable represents.
- Name variables based on *what* they are, not *how* they are implemented.
- Good naming should minimize the need for explanatory comments.
...
### Project-Specific Modularity & Comment Rules

#### Modularity
Code design must answer:
- What is the single responsibility of this module?
- Why does this function belong here?
- At which layer does this business rule reside?
- Which dependencies are strictly necessary?
- Why is the same information represented in multiple places?

Objectives:
- Clean separation of concerns
- Elimination of duplication
- Simplification of dependency trees
- Reduced cost of future changes

#### Commenting Policy
Comments should be treated as a last resort.

Preferred order of clarity:
- Descriptive naming
- Small, focused functions and modules
- Brief explanatory comments (only when necessary)

Comments are useful for:
- Explaining *why* a particular approach was chosen
- Documenting domain quirks or unexpected requirements
- Noting external system/library constraints
...
### Project-Specific SOLID & Quality Rules

#### General Code Quality Guidelines
Every code change must aim to improve:
- Readability
- Maintainability
...
- Redundant wrappers or helper factories
- Purely theoretical cleanliness that makes practical usage harder

Code should be written to be **simple, easy to understand, and safe to modify**, rather than appearing "clever."
...
- Evaluate Strategy, Factory, Builder, Adapter,
  Facade, Repository, Dependency Injection,
  Command and Observer patterns.

Do not introduce a pattern unless it reduces
complexity or duplication.

Prefer simpler code over enterprise-style abstractions.


#### SOLID and Code Smells
Actively detect and refactor the following issues:
- Large, bloated functions and classes
...
- High risk of shotgun surgery
- Hardcoded magic numbers and strings
- Inconsistent representations of domain knowledge

Every refactoring effort must add real value. Do not introduce abstractions solely to make the code look more "enterprise."
...
This section outlines the development process, testing strategies, and instructions on how to run tests in this project.

### Determine the Nature of the Change
Before starting any task, categorize the work as one of the following:
- **Refactoring**
- **New feature development**
- **Bug fixing**
- **Behavioral/rule change**
- **Performance optimization**
- **Architectural adjustment / modularization**
- **Test infrastructure improvement**

Clarify the category before editing any code, and align your workflow accordingly.

### Behavior Preservation Rule
- **During Refactoring:** Keep external behavior identical. The code must produce the same outputs and run the same flows, preserving existing business rules.
- **During Features, Bug Fixes, or Product Changes:** Behavior changes are expected. However, these changes must be **deliberate and controlled**, documented clearly, and covered by updated test assertions.

### Workflow Sequence
Follow this order of execution for every task:
- **Analyze:** Examine the codebase first; do not make large structural changes blindly.
- **Categorize:** Identify the task type.
- **Plan:** Outline a brief implementation plan.
- **Assess:** Identify affected files and potential regression risks.
- **Implement:** Write code in a controlled, step-by-step manner.
- **Test:** Add new test cases or update existing tests.
- **Verify:** Run the test suite and ensure all validations pass.
- **Report:** Document the outcome and impacts in a short technical summary.

---

### Running Tests in the Project
This project uses `pytest` and `pytest-qt` for testing. To run PyQt/GUI tests in a headless (headless shell) or terminal environment, you must specify the offscreen platform parameter.

#### Command to Run Tests:
Run the following command in the terminal to execute the test suite:
```bash
QT_QPA_PLATFORM=offscreen .venv/bin/python -m pytest
```

#### Git Pre-Push Hook & CI:
- Set up git hooks using `cv-git-hook-kur` or by linking `.githooks/pre-push`.
- Local pushes (`git push`) automatically execute pytest beforehand.
- GitHub Actions runs the same test suite on Pull Requests to verify integration safety.
- Do not push changes if any test is failing.

---

### Task-Specific Test Strategies
- **Refactoring:** Focus heavily on regression and characterization tests. All existing tests must pass successfully.
- **New Feature:** Add test cases covering new behaviors, validation boundaries, and edge cases.
- **Bug Fix:** Write regression tests that reproduce the bug before fixing it, ensuring it does not recur.
- **Behavioral Change:** Update test assertions to reflect the new expected behavior, explicitly separating old and new expectations.

### Guidelines for Features, Fixes, and Refactoring

#### New Features
- Do not break existing structures.
- Define new behavior explicitly.
- Keep business rules consolidated; preserve the Single Responsibility Principle.
- Verify that tests clearly express the new feature's behavior.

#### Bug Fixing
- Identify the scope and address the root cause rather than patching symptoms.
...
- Avoid modifying core logic for the sake of styling or cosmetic cleanup.

### Post-Change Verification & Reporting

#### Verification Checklist:
- Is the code consistent and readable?
- Are tests updated and passing?
- Have magic numbers and strings been minimized?
- Has technical debt been reduced?

#### Technical Report Requirement:
After completing any meaningful change, provide a concise technical summary detailing:
- Type of task (Refactor, Bug Fix, etc.)
- Modifications made and affected files
- Added/updated test scenarios
- Behavior preservation status (and description of any deliberate changes)
- Remaining risks or technical debt

### Decision Hierarchy
Prioritize decisions in the following order:
- Correctly understanding the true goal of the task
- Making controlled and incremental modifications
- Keeping code clean, readable, and maintainable
- Centralizing magic numbers and strings
- Preserving or increasing testability
